...
(function () {
  ...
  const state = {
    ...
    chatDraft: "", modal: null, sidebarOpen: false, chatListOpen: true,
  ...
  function renderChat() {
    ...
    return `
    <div class="chat-wrap ${state.chatListOpen ? "" : "list-hidden"}">
      ...
      <div class="chat">
        <div class="chat-head">
          ...
          <button class="btn btn-sm toggle-list-btn" data-action="toggle-chat-list">${state.chatListOpen ? "◀ Hide" : "▶ Chats"}</button>
  ...
  function onClick(e) {
    ...
    switch (a) {
      ...
      case "toggle-chat-list": state.chatListOpen = !state.chatListOpen; render(); break;
      case "signout": signOut(); break;
      case "toggle-task": if (!e.target.closest('[data-action="del-task"]')) toggleTask(id, el.dataset.done === "true"); break;
      case "del-task": e.stopPropagation(); if (!confirm("Delete this task? This can't be undone.")) return; delTask(id); break;
      ...
      case "del-client": if (!confirm("Delete this client? This can't be undone.")) return; delClient(id); break;
      case "new-note": state.modal = { type: "note" }; render(); break;
      case "save-note": doSaveNote(); break;
      case "del-note": if (!confirm("Delete this note? This can't be undone.")) return; delNote(id); break;
      ...
      case "del-event": e.stopPropagation(); if (!confirm("Delete this event?")) return; delEvent(id); break;
